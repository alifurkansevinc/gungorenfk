...
export default async function TransferlerPage() {
  const supabase = await createClient();
  const { data: transfers } = await supabase
    .from("transfers")
    .select("id, player_name, player_image_url, from_team_name, from_team_league, to_team_name, to_team_league, transfer_date")
    ...
    <div className="min-h-screen bg-[#f8f8f8]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        {(!transfers || transfers.length === 0) ? (
          <div className="rounded-2xl border-2 border-dashed border-siyah/15 bg-beyaz p-12 text-center text-siyah/60">
            <p className="font-medium">Henüz transfer kaydı yok.</p>
            ...
            {transfers.map((t) => {
              ...
              return (
                <li key={t.id} className="overflow-hidden rounded-2xl border border-siyah/10 bg-beyaz shadow-lg shadow-siyah/5">
                  {/* Üçlü blok: Geldiği takım → Oyuncu → Gittiği takım */}
                  <div className="grid grid-cols-[1fr_auto_1fr] gap-0 min-h-[200px] sm:min-h-[240px]">
                    {/* Sol: Geldiği takım */}
                    <div className="flex flex-col justify-center border-r border-siyah/10 bg-gradient-to-b from-siyah/5 to-transparent px-4 py-6 text-center sm:px-8">
                      <p className="text-xs font-semibold uppercase tracking-wider text-bordo">Geldiği takım</p>
                      <p className="font-display mt-2 text-base font-bold text-siyah sm:text-xl">{t.from_team_name}</p>
                      {t.from_team_league && (
                    ...
                    {/* Orta: Oyuncu */}
                    <div className="flex flex-col items-center justify-center px-4 py-6 sm:px-8">
                    ...
                    {/* Sağ: Gittiği takım */}
                    <div className="flex flex-col justify-center border-l border-siyah/10 bg-gradient-to-b from-bordo/5 to-transparent px-4 py-6 text-center sm:px-8">
                      <p className="text-xs font-semibold uppercase tracking-wider text-bordo">Gittiği takım</p>
                      <p className="font-display mt-2 text-base font-bold text-siyah sm:text-xl">{t.to_team_name}</p>
                      {t.to_team_league && (
