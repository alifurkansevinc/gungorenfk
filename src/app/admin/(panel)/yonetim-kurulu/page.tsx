...
import { YonetimSilButton } from "./YonetimSilButton";
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-siyah">Yönetim Kurulu</h1>
          <p className="mt-1 text-siyah/70">Başkan, YK üyeleri ve danışmanlar. Sitede Kulübümüz → Yönetim Kurulu sayfasında gösterilir.</p>
        </div>
        <Link href="/admin/yonetim-kurulu/yeni" className="rounded-lg bg-bordo px-4 py-2 text-sm font-semibold text-beyaz hover:bg-bordo-dark">
          + Yeni üye
        </Link>
      </div>
              ...
              members.map((m) => (
                <tr key={m.id} className="border-t border-siyah/5 hover:bg-siyah/[0.02]">
                  ...
                  <td className="px-4 py-3 flex gap-3">
                    ...
                    <YonetimSilButton id={m.id} name={m.name} />
                  </td>
                </tr>
              ))
            )}
