...
import type { ReceiptData } from "@/lib/orders";

export default function OdemeBasariliContent({
  initialReceipt,
  orderNumber,
  levelUp,
}: {
  ...
  const pickupDateFormatted = receipt?.pickupDate
    ...
    : "";
  const qrUrl =
    typeof window !== "undefined" && isStorePickup && receipt?.pickupCode
  ...
  const handlePdfDownload = useCallback(() => {
    if (!isStorePickup || !receipt?.pickupCode) return;
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    const url = `https://api.qrserver.com/v1/create-qr-code/?size=256x256&data=${encodeURIComponent(`${origin}/admin/teslim-al?code=${receipt.pickupCode}`)}`;
    ...
    w.document.write(`
      ...
      <img src="${url}" alt="QR" width="256" height="256" />
  ...
  }, [isStorePickup, receipt?.pickupCode, receipt?.orderNumber, orderNumber, pickupDateFormatted]);
      ...
      <div className="rounded-2xl border-2 border-green-200 bg-green-50 p-6 sm:p-8">
        ...
        {orderNumber && !receipt && (
          <p className="mt-6 text-center text-amber-700">
            Sipariş numarası: <strong>{orderNumber}</strong>. Sipariş henüz onaylanmamış olabilir veya bulunamadı. Benim Köşem’den kontrol edebilirsiniz.
          </p>
        )}
        {!orderNumber && (
          <p className="mt-6 text-center text-siyah/70">Sipariş numarası bulunamadı. Sorun yaşıyorsanız iletişime geçin.</p>
        )}
        ...
        {receipt && (
          <>
            ...
            {receipt.deliveryMethod !== "store_pickup" && (
