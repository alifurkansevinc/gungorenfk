...
function StoreQrContent() {
  ...
  const qrData = origin ? `${origin}/admin/teslim-al?code=${encodeURIComponent(code)}` : "";
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col items-center justify-center px-4">
      <div className="max-w-md w-full rounded-2xl border border-siyah/10 bg-beyaz p-8 shadow-lg text-center">
        ...
        <h1 className="mt-4 font-display text-xl font-bold text-siyah">Mağazadan teslim</h1>
        <p className="mt-2 text-siyah/70 text-sm">
          Mağazada bu QR kodu gösterin; personel okutunca siparişiniz teslim alınır.
        </p>
        {orderNumber && (
          <p className="mt-1 text-sm font-semibold text-siyah/80">Sipariş: {orderNumber}</p>
        )}
